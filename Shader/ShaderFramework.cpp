...
#define ASPECT_RATIO (WIN_WIDTH/(float)WIN_HEIGHT)		//종횡비
...
{
	// 텍스처 로딩
	gpEarthDM = LoadTexture(L"Earth.jpg");
	if (!gpEarthDM)
	{
		return false;
	}
	// 쉐이더 로딩
	gpColorShader = LoadShader(L"ColorShader.fx");
	if (!gpColorShader)
	{
		return false;
	}
	gpTextureMappingShader = LoadShader(L"TextureMapping.fx");
